...
aes_ctx_t *aes_ctr_copy(const aes_ctx_t *src) {
    if (!src || !src->cipher_ctx) return NULL;
    aes_ctx_t *ctx = malloc(sizeof(*ctx));
    if (!ctx) return NULL;
    ctx->cipher_ctx = EVP_CIPHER_CTX_new();
    if (!ctx->cipher_ctx) {
        free(ctx);
    ...
    if (!EVP_CIPHER_CTX_copy(ctx->cipher_ctx, src->cipher_ctx)) {
        EVP_CIPHER_CTX_free(ctx->cipher_ctx);
        free(ctx);
        return NULL;
    }
    memcpy(ctx->key, src->key, AES_128_BLOCK_SIZE);
    memcpy(ctx->iv, src->iv, AES_128_BLOCK_SIZE);
    ctx->direction = src->direction;
    ctx->block_offset = src->block_offset;
    return ctx;
}
